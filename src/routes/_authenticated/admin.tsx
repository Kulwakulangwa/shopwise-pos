...
export const Route = createFileRoute("/_authenticated/admin")({
  ...
  head: () => ({
    meta: [
      { title: "Admin Control Room — My Shop" },
...
function Admin() {
  ...
  const updateUserRole = async (userId: string, roleId: string) => {
    try {
      await db.from("users").update({ role_id: roleId }).eq("id", userId);
  ...
  const saveSetting = async () => {
    ...
    try {
      // Handle JSON settings like credit_limit_action
      let valueToSave = settingValue;
      if (editingSetting.key === "credit_limit_action") {
        // Keep as JSON string
  ...
  const importBackup = async (file: File) => {
    toast.info("Restore will overwrite existing data. Confirm?");
    if (!confirm("This will OVERWRITE all existing data. Are you sure?")) return;
    try {
      const text = await file.text();
      const backup = JSON.parse(text);
      // For each table, delete all and insert backup
      // In production, use a proper migration tool. This is simplified.
      ...
      <Dialog open={settingsDialogOpen} onOpenChange={setSettingsDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              <Label>Value</Label>
              {editingSetting?.key === "credit_limit_action" ? (
                <Select value={settingValue} onValueChange={setSettingValue}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select action" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value='{"mode":"block"}'>Block credit sale</SelectItem>
                    <SelectItem value='{"mode":"warn"}'>Warn only</SelectItem>
                  </SelectContent>
                </Select>
              ) : (
                <Input
        ...
        <TableBody>
          {users.map((u: any) => (
            <TableRow key={u.id}>
              ...
              <TableCell>{u.last_sign_in_at ? date(u.last_sign_in_at) : "Never"}</TableCell>
      ...
      <Table>
        ...
        <TableBody>
          {settings.map((s: any) => {
            ...
            if (s.key === "credit_limit_action") {
              try {
                const parsed = JSON.parse(s.value);
                displayValue = parsed.mode === "block" ? "Block credit sale" : "Warn only";
              } catch {
